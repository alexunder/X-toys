...
static void RasterizeTriangleInterval(const ShadedGrid &sg, int u0, int v0, int u1, int v1,
                              int u2, int v2, int interval, Bucket *bucket)
{
    ...
    {
        //Compute the bbox for each interval
        float ct0 = i * delta;
        float ct1 = (i + 1) * delta;

        vta[0] = vx0[0] * (1 - ct0) + vx0[1] * ct0;
        vtb[0] = vx1[0] * (1 - ct0) + vx1[1] * ct0;
        vtc[0] = vx2[0] * (1 - ct0) + vx2[1] * ct0;
        vta[1] = vx0[0] * (1 - ct1) + vx0[1] * ct1;
        vtb[1] = vx1[0] * (1 - ct1) + vx1[1] * ct1;
        vtc[1] = vx2[0] * (1 - ct1) + vx2[1] * ct1;

        // Now compute the bounding box of the triangle on the screen in
        // floating-point pixel coordinates.
        float xMin = vta[0].x2d();
        float xMax = vta[0].x2d();
        float yMin = vta[0].y2d();
        float yMax = vta[0].y2d();

        for (int j = 0; j < 2; j++)
        {
            xMin = std::min(xMin, std::min(vta[j].x2d(), std::min(vtb[j].x2d(), vtc[j].x2d())));
            yMin = std::min(yMin, std::min(vta[j].y2d(), std::min(vtb[j].y2d(), vtc[j].y2d())));
            xMax = std::max(xMax, std::max(vta[j].x2d(), std::max(vtb[j].x2d(), vtc[j].x2d())));
            yMax = std::max(yMax, std::max(vta[j].y2d(), std::max(vtb[j].y2d(), vtc[j].y2d())));
        }
...
void Rasterizer3D::Rasterize(const std::vector<ShadedGrid> &grids,
                        int numIntervals, Bucket *bucket)
{
...
#if MBRAST_MODE == BASIC_RAST_3D
                RasterizeTriangle(sg, u, v, u+1, v, u+1, v+1, bucket);
                RasterizeTriangle(sg, u, v, u+1, v+1, u, v+1, bucket);
#elif MBRAST_MODE == INTER_RAST_3D
                RasterizeTriangleInterval(sg, u, v, u+1, v, u+1, v+1, numIntervals, bucket);
                RasterizeTriangleInterval(sg, u, v, u+1, v+1, u, v+1, numIntervals, bucket);
#endif
            }
        }
    }
